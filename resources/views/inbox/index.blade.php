<x-app-layout>
            ...
            <div class="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700" id="conversation-list">
                @forelse($conversations as $conv)
                    <a href="{{ route('inbox.conversations.show', $conv) }}"
                       ...
                       class="conv-item block px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        ...
                        @if($conv->assignedUser)
                            <div class="text-[10px] text-gray-400 dark:text-gray-500 mt-1 truncate">
    ...
    function loadConversation(convId, event) {
        ...
        document.querySelectorAll('.conv-item').forEach(function(el) {
            el.classList.remove('bg-indigo-50', 'dark:bg-indigo-900/20', 'border-l-2', 'border-indigo-500');
        });
        var clicked = document.querySelector('[data-conv-id="' + convId + '"]');
        if (clicked) clicked.classList.add('bg-indigo-50', 'dark:bg-indigo-900/20', 'border-l-2', 'border-indigo-500');
    ...
    function renderChat(data) {
        ...
        // Input
        if (c.is_open) {
            html += '<div class="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">';
            html += '<form onsubmit="sendMessage(event, ' + c.id + ')" class="flex gap-2 items-end">';
            html += '<div class="flex-1"><textarea id="msg-body-' + c.id + '" rows="1" required placeholder="Escribe un mensaje..." class="w-full text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:ring-indigo-500 focus:border-indigo-500 resize-none"></textarea></div>';
            html += '<div class="flex flex-col gap-1">';
            html += '<button type="submit" class="px-3 py-2 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg">Enviar</button>';
            html += '<button type="button" onclick="sendNote(' + c.id + ')" class="px-3 py-2 text-xs font-medium text-yellow-700 bg-yellow-100 hover:bg-yellow-200 dark:text-yellow-300 dark:bg-yellow-900/30 rounded-lg">Nota</button>';
            html += '</div></form></div>';
        } else {
    ...
    function renderMessage(msg) {
        ...
        if (msg.type === 'internal_note') {
            ...
            html += '<div class="max-w-md px-3 py-2 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-xs text-yellow-700 dark:text-yellow-300">';
            ...
            html += '<div class="max-w-md px-4 py-2 rounded-2xl rounded-bl-sm bg-white dark:bg-gray-700 shadow-sm text-sm text-gray-800 dark:text-gray-200">';
            html += esc(msg.body) + '<div class="text-[10px] text-gray-400 mt-1 text-right">' + esc(msg.time) + '</div></div></div>';
        } else {
            html += '<div data-msg-id="' + msg.id + '" class="flex justify-end">';
            html += '<div class="max-w-md px-4 py-2 rounded-2xl rounded-br-sm bg-indigo-600 text-white text-sm shadow-sm">';
            html += esc(msg.body) + '<div class="text-[10px] text-indigo-200 mt-1 text-right">' + esc(msg.user_name || 'Agent') + ' · ' + esc(msg.time) + '</div></div></div>';
        }
        return html;
    }
    ...
    function appendOptimistic(body, type) {
        ...
        if (type === 'internal_note') {
            ...
            div.innerHTML = '<div class="max-w-md px-4 py-2 rounded-2xl rounded-br-sm bg-indigo-600 text-white text-sm shadow-sm">' + esc(body) + '<div class="text-[10px] text-indigo-200 mt-1 text-right">Yo · ' + time + '</div></div>';
        }
