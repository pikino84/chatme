    ...
    @foreach($messages as $msg)
        @if($msg->isInternalNote())
            <div class="flex justify-center" data-msg-id="{{ $msg->id }}">
                <div class="max-w-[85%] sm:max-w-md px-3 py-2 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-xs text-yellow-700 dark:text-yellow-300">
        ...
        @elseif($msg->isInbound())
            <div class="flex justify-start" data-msg-id="{{ $msg->id }}">
                <div class="max-w-[85%] sm:max-w-md px-4 py-2 rounded-2xl rounded-bl-sm bg-white dark:bg-gray-700 shadow-sm text-sm text-gray-800 dark:text-gray-200">
                    {{ $msg->body }}
        ...
        @else
            <div class="flex justify-end" data-msg-id="{{ $msg->id }}">
                <div class="max-w-[85%] sm:max-w-md px-4 py-2 rounded-2xl rounded-br-sm bg-indigo-600 text-white text-sm shadow-sm">
                    {{ $msg->body }}
